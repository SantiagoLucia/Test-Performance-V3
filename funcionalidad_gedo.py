...
from config import USER,PASSW,URL_GEDO
...
def test_firmar_gedo(acronimo):
    ...
    while intentos < 3:
        try:
            ...
            buscar_elemento(driver, By.CSS_SELECTOR, eug.input_acronimo).send_keys(acronimo)
            buscar_elemento(driver, By.CSS_SELECTOR, eug.boton_producir_yo_mismo).click()
            buscar_elemento(driver, By.XPATH, eug.input_motivo).send_keys('Documento de prueba. Carece de motivacion administrativa')
            time.sleep(1)
            ...
            if acronimo == 'TESTL':
                ...
                editor_body.send_keys('Documento de prueba. Carece de motivacion administrativa.')
                #Salgo del ckeditor
                driver.switch_to.default_content()

            if acronimo == 'TESTT':
                buscar_elemento(driver, By.CSS_SELECTOR, "input[name='pers']").send_keys('Sr.')
                buscar_elemento(driver, By.CSS_SELECTOR, "input[name='nomyape']").send_keys('Prueba')
                buscar_elemento(driver, By.CSS_SELECTOR, "input[name='dni']").send_keys('12345678')
                buscar_elemento(driver, By.CSS_SELECTOR, "input[name='mes']").send_keys('Enero')
                buscar_elemento(driver, By.CSS_SELECTOR, "input[name='anio']").send_keys('2022')
                buscar_elemento(driver, By.CSS_SELECTOR, "input[name='dependencia']").send_keys('Dirección Provincial de Mejora Administrativa')

            if acronimo == 'TESTI':
                buscar_elemento(driver, By.CSS_SELECTOR, 'input[name="file"]').send_keys(os.path.abspath('imagen.png'))
            ...
            nro_gedo = buscar_elemento(driver, By.XPATH, eug.span_nro_gedo).text.split()[14]
    ...
    while intentos < 3:
        try:
            ...
            for num in range(11,16):
                buscar_elemento(driver, By.XPATH, f"(//span[@class='z-listitem-checkable z-listitem-checkbox'])[{num}]").click()
...
def test_buscar_documento(nro_gedo):
    ...
    while intentos < 3:
        try:
            ...
            buscar_elemento(driver, By.CSS_SELECTOR, cas.input_passw).send_keys(PASSW)
            buscar_elemento(driver, By.CSS_SELECTOR, cas.boton_acceder).click()
            buscar_elemento(driver, By.XPATH, "//input[@placeholder='Ingrese el número']").send_keys(nro_gedo)
            time.sleep(1)
            buscar_elemento(driver, By.XPATH, "//button[@title='Buscar']").click()
            t_ini = time.perf_counter()
            buscar_elemento(driver, By.XPATH, "//div[contains(text(),'Cantidad de registros encontrados: 1')]")
            t_fin = time.perf_counter()
            t_total = t_fin - t_ini
            t_totals = str("%.2f" % t_total)
            nodo = buscar_elemento(driver, By.CSS_SELECTOR, eug.nodo).text
...
if __name__ == '__main__':
    ...
    pbar = tqdm(total=5, ncols=90, colour='green', bar_format = "{l_bar}{bar}| {n_fmt}/{total_fmt}")
