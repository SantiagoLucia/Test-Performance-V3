...
from config import USER,PASSW,URL_EE
...
def test_caratular_expediente():
    ...
    while intentos < 3:
        try:
            ...
            buscar_elemento(driver, By.CSS_SELECTOR, eue.boton_caratular_interno).click()
            buscar_elemento(driver, By.CSS_SELECTOR, eue.textarea_motivo_interno).send_keys('Expediente de prueba. Carece de motivacion administrativa.')
            buscar_elemento(driver, By.CSS_SELECTOR, eue.textarea_motivo_externo).send_keys('Expediente de prueba. Carece de motivacion administrativa.')
            buscar_elemento(driver, By.XPATH, eue.input_codigo_tramite).send_keys('TEST0001')
            buscar_elemento(driver, By.XPATH, eue.textarea_descripcion_adicional).send_keys('Expediente de prueba. Carece de motivacion administrativa.')
            buscar_elemento(driver, By.XPATH, eue.boton_caratular).click()
            t_ini = time.perf_counter()
            nro_exped = buscar_elemento(driver, By.CSS_SELECTOR, eue.span_nro_expediente).text
...
def test_guarda_temporal():
    ...
    while intentos < 3:
        try:
            ...
            time.sleep(1)
            driver.switch_to.frame(buscar_elemento(driver, By.XPATH, eue.iframe_ckeditor))
            buscar_elemento(driver, By.CSS_SELECTOR, "body p").send_keys('Expediente de prueba. Carece de motivacion administrativa.')
            ...
            t_ini = time.perf_counter()
            buscar_elemento(driver, By.CSS_SELECTOR, '.z-messagebox-icon.z-messagebox-information.z-div')
            nro_exped = buscar_elemento(driver, By.CSS_SELECTOR, eue.span_nro_expediente).text
            nro_exped = nro_exped.split()[2] + ' ' + nro_exped.split()[3].replace(',', '')
            t_fin = time.perf_counter()
...
def test_solicitud_caratulacion():
    ...
    while intentos < 3:
        try:
            ...
            time.sleep(1)
            buscar_elemento(driver, By.CSS_SELECTOR, eue.input_usuario).send_keys("Test Performance ")
            time.sleep(3)
