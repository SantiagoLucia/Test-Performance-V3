...
from config import USER,PASSW,URL_CCOO
...
def test_firmar_ccoo(acronimo):
    ...
    while intentos < 3:
        try:
            ...
            buscar_elemento(driver, By.CSS_SELECTOR, euc.input_acronimo).send_keys(acronimo)
            buscar_elemento(driver, By.CSS_SELECTOR, euc.boton_producir_yo_mismo).click()
            buscar_elemento(driver, By.XPATH, euc.input_motivo).send_keys('Documento de prueba. Carece de motivacion administrativa')
            time.sleep(1)
            ...
            nro_gedo = buscar_elemento(driver, By.XPATH, euc.span_nro_gedo).text.split()[14]
...
if __name__ == '__main__':
    ...
    pbar = tqdm(total=2, ncols=90, colour='green', bar_format = "{l_bar}{bar}| {n_fmt}/{total_fmt}")
